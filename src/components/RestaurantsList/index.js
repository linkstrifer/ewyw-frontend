...
export default function RestaurantsList(){
  ...
  return(
    <div className='restaurantList'>
      ...
      { restaurants && restaurants.length > 0 && restaurants.map(({name, direction, phone, _id}) => {
        return(
          <div className='restaurantsCard'>
            <span><strong>Nombre:</strong> {name}</span><br/>
            <span><strong>Direccion:</strong> {direction}</span><br/>
            <span><strong>Telefono:</strong> {phone}</span><br/>
            <a href={`/restaurantprofile/${_id}`}><Button type='button'>Ir a perfil</Button></a>
          </div>
        )
        })
      }
